...
- **Update 2026-09-26**: the read-path cost is solved by `explorer_dataset()` (BUG-033), which dedupes on the fly; only storage and write size remain. Note that Red Hat impacts can differ per CVE (66 of 578 mappings on production), so storage dedupe must keep per-mapping references rather than a single advisory-level list.
